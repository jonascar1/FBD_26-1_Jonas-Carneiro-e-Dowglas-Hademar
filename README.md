# Condominium Management Database
This repository contains the practical database project developed for the **Fundamentals of Databases** course.
The project consists of the analysis, modeling, and implementation of a relational database for a condominium management system. The development process covers domain understanding, conceptual modeling, relational mapping, SQL implementation, and query creation.
## Project Structure
For a better understanding of the project, follow the documents in the order below:
1. **Conceptual Data Dictionary**  
   [Conceptual Data Dictionary](docs/conceptual_data_dictionary.md)
2. **Entity-Relationship Diagram (ERD)**  
   [ER Diagram](media/er_diagram.png)
3. **Relational Mapping**  
   [Relational Mapping](docs/relational_mapping.md)
4. **Relational Data Dictionary and SQL Definition**  
   [Relational Data Dictionary With SQL](docs/relational_data_dictionary_with_sql.md)
5. **Database Schema Creation**  
   [Create Tables](database/create_tables.sql)
6. **Sample Data Insertion**  
   [Insert Sample Data](database/insert_sample_data.sql)
7. **SQL Query Examples**  
   [Queries Examples](database/queries_examples.sql)
## Technologies and Tools
![SQL](https://img.shields.io/badge/SQL-Implemented-blue)   
![BrModelo](https://img.shields.io/badge/BrModelo-ER%20Modeling-green)
## How to Run the Database
1. Create a new database in your preferred DBMS.
2. Execute the schema creation script:
   [Create Tables](database/create_tables.sql)
3. Populate the database with sample data:
   [Insert Sample Data](database/insert_sample_data.sql)
4. Run the example queries to validate the database structure and explore the stored data:
   [Queries Examples](database/queries_examples.sql)
## Academic Context
This project was developed as part of the **Fundamentals of Databases** course and demonstrates the complete database development process, from conceptual modeling to SQL implementation.
## Presentation Video
The presentation video for this project is available at:
[Presentation Video]
